...
static inline void fill(uint8_t* target, uint8_t color) {
	for (int i = LCD_WIDTH*LCD_HEIGHT; i > 0; i--) {
		*(target++) = (*target & 0x7F) | color;
	}
}

static void render(uint8_t (&framebuffer)[LCD_HEIGHT][LCD_WIDTH], LCD::State& lcd) {
	if (!lcd.display_enable) {
		fill(&framebuffer[0][0], 0x00);
		return ;
	} else if (lcd.all_on) {
		fill(&framebuffer[0][0], 0x80);
		return ;
	}
...
void LCD::clock(Machine::State& cpu, int osc3) {
	...
	while (cpu.lcd.overflow >= OSC3_SPEED) {
		if (cpu.lcd.scanline >= 0x40) {
			render(cpu.buffers.framebuffer, cpu.lcd);
