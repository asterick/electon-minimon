		...
		for (int i = 23; i >= 0; i--) {
			...
			for (int s = 0; s < 2; s++) {
				for (int x = 0; x < 8; x++, address++, dx++) {
					...
					uint16_t mask = cpu_read8(cpu, (address^invert) +  0, TRACE_SPRITE_DATA) 
					             | (cpu_read8(cpu, (address^invert) +  8, TRACE_SPRITE_DATA) << 8);
					uint16_t draw = cpu_read8(cpu, (address^invert) + 16, TRACE_SPRITE_DATA) 
					             | (cpu_read8(cpu, (address^invert) + 24, TRACE_SPRITE_DATA) << 8);
